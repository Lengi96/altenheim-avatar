...
router.post('/', validate(chatSchema), async (req: Request, res: Response) => {
  ...
  if (mode === 'pfleger' && auth.role === 'resident') {
    res.status(403).json({ error: 'Pfleger-Modus ist nur fÃ¼r Mitarbeiter.' });
    return;
  }
  ...
  if (convId) {
    ...
    if (conv.mode !== mode) {
      res.status(400).json({ error: 'conversationId passt nicht zum gewÃ¤hlten Modus.' });
      return;
    }
  } else {
    const [conv] = await db
  ...
  if (!convId || !targetResidentId) {
    res.status(500).json({ error: 'GesprÃ¤ch konnte nicht initialisiert werden.' });
    return;
  }
